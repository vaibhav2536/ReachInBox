      ...
      <div className="border-b-2 dark:border-[#33383F] border-[#E0E0E0] w-full flex justify-between px-8 py-4">
        ...
        <div className="flex items-center space-x-3">
          <div className="flex dark:bg-[#1F1F1F] bg-white border dark:border-[#343A40] items-center text-black dark:text-white rounded-md py-2 px-3 text-sm">
            <GoDotFill className="text-yellow-500 text-xl" /> Meeting Completed <SlArrowDown className="ml-2" />
          </div>
          ...
          <div className="dark:bg-[#1F1F1F] border bg-white text-black dark:text-white dark:border-[#343A40] rounded-md py-2 px-3 text-sm">...</div>
        </div>
      </div>
      ...
      <div className="py-8 mx-8 relative flex justify-center items-center">
        ...
        <div className="absolute inset-0 flex justify-center items-center">
          <div className="dark:bg-[#171819] bg-[#E0E0E0] text-black dark:text-white px-4 py-1 text-sm flex items-center space-x-1">
            <MdOutlineExpand className="mr-3 text-xl text-[#AEAEAE]" /> View all <span className="text-blue-500">4</span>
            <span>replies</span>
          </div>
        </div>
      </div>
      <div className="mx-8">{showPopUp && <CustomMail threadId={selectedThread} onClose={() => setShowPopUp(false)} />}</div>
      <div className="cursor-pointer flex items-center fixed bottom-0 ml-10 mb-10 bg-gradient-to-r from-[#4B63DD] to-[#0524BFFC] rounded-md px-10 py-2" onClick={togglePopUp}>
        <FaReply className="mr-2 text-xl" /> Reply
      </div>
      {showDelete && <DeletePopUp onCancel={() => setShowDelete(false)} onDelete={handleDelete} />}
    </div>
  );
};
  ...
  return (
    <div className="dark:bg-[#141517] bg-white border dark:border-[#343A40] mx-8 rounded-md my-3">
      <div className="p-4">
        ...
        <div className="py-4 dark:text-[#E1E0E0] text-[#172B4D] w-2/3" dangerouslySetInnerHTML={{ __html: body }} />
      </div>
    </div>
  );
};
